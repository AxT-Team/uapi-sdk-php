...
```php
getGithubUser($user, $activity, $activity_scope, $org): \OpenAPI\Client\Model\GetGithubUser200Response
```
...
try {
    $result = $apiInstance->getGithubUser($user, $activity, $activity_scope, $org);
    print_r($result);
} catch (Exception $e) {
