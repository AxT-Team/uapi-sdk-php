...
```php
getMiscHolidayCalendar($date, $month, $year, $timezone, $holiday_type, $include_nearby, $nearby_limit, $exclude_past): \OpenAPI\Client\Model\GetMiscHolidayCalendar200Response
```
...
$exclude_past = true; // bool | 传 true 时，会过滤今天之前已经过去的节日。默认 false。

try {
    $result = $apiInstance->getMiscHolidayCalendar($date, $month, $year, $timezone, $holiday_type, $include_nearby, $nearby_limit, $exclude_past);
    print_r($result);
} catch (Exception $e) {
...
```php
getMiscHotboard($type, $time, $keyword, $time_start, $time_end, $limit): \OpenAPI\Client\Model\GetMiscHotboard200Response
```
...
try {
    $result = $apiInstance->getMiscHotboard($type, $time, $keyword, $time_start, $time_end, $limit);
    print_r($result);
} catch (Exception $e) {
